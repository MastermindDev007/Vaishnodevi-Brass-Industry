...
<head>
	...
	<title>Sub-Category || Vaishnodevi Brass Industries</title>
	...
	<link rel="stylesheet" href="includes/css/sub_category.css">
...
<body>
	...
	<div class="category_content">
		...
		<div class="button_content">
			<button onclick="document.getElementById('id01').style.display='block'" style="width:auto;" class="btn_cat">Add Sub-Category</button>
			<div id="id01" class="modal">
				<form class="modal-content animate" action="#" method="post" enctype="multipart/form-data">
					...
					<div class="category">
						<p class="p5">SUB-CATEGORY</p>
						<p class="p6">Axdd Sub-Category</p>
							<label for="Sub-Category Name" class="first">Sub-Category Name:</label>
							<input type="text" name="sub_category_name" id="sub_Category_Name" placeholder="Enter Category Name Here" class="i1" required>
							...
							<input type="file" name="sub_category_image" id="sub_category_image" class="i3" required>
						
							<label for="Description" class="fourth">Description:</label>
							<textarea name="description" cols="143" rows="4" class="text1"></textarea>
						
							<input type="submit" class="submit" name="add_sub_category" value="ADD">
					</div>
			...
			<table class="table table-hover table-borderd table-striped shadow-lg">
				...
				<tbody>
					<?php
						...
						$st = "SELECT * FROM `sub_category` WHERE 1";
						$res = mysqli_query($cn, $st) or die("Not Inserted");
						while ($fld=mysqli_fetch_array($res)) {
					?>
						<tr>
							<td><p style="text-align: center;"><?php echo $fld['id']?></p></td>
							<td><?php echo $fld['sub_category_name']?></td>
							<td>
								<?php
									$a = $fld["category_name"];
							...
							<td>
								<img src="<?php echo $fld['sub_category_image']?>" width="150px" alt="Uploaded Image">
							</td>
							<td><?php echo $fld['sub_cat_description']?></td>
							<td><a href="update_sub_category.php?id=<?php echo $fld['id']?>" class="btn btn-success">Update</a></td>
							<td><a href="delete_sub_category.php?id=<?php echo $fld['id']?>" class="btn btn-danger">Delete</a></td>
						</tr>
					<?php
						}
					?>
