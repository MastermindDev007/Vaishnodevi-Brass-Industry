...
if (isset($_POST['update_brochure'])) {
    ...
    $new_image = $_FILES['new_pdf']['name'];
    $old_image = $_POST['old_pdf'];
    $target_dir = "/assets/header/uploads/"; // Specify the directory where you want to save the uploaded files
    ...
    if (move_uploaded_file($_FILES['new_pdf']['tmp_name'], $target_file)) {
        ...
        // Update the database
        $st = "UPDATE brochure SET brochure='$update_filename' WHERE id = '$stud_id'";
    ...
    header('Location: http://localhost/vaishnodevi/admin/brochure.php');
}
?>
