...
<body>
     ...
     if (isset($_GET['id'])) {
          ...
          $st = "SELECT * FROM `email` WHERE `id` = '$id'";
     ...
     if (isset($_POST['update_email'])) {
          ...
          $st = "UPDATE `email` SET `email`='$email', `status`='$status' WHERE id = '$id'";
          $res = mysqli_query($cn, $st) or die("Query Failed");
          header('Location: http://localhost/vaishnodevi/admin/email.php');
     }
     ?>

     <form action="update_email.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
          ...
          <br>
          <input type="submit" class="btn btn-success" name="update_email" value="UPDATE">
     </form>
</body>
